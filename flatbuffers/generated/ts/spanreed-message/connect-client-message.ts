...
export class ConnectClientMessage implements flatbuffers.IUnpackableObject<ConnectClientMessageT> {
  ...
  unpack(): ConnectClientMessageT {
    return new ConnectClientMessageT(
      this.connectionString(),
      this.bb!.createScalarList<number>(this.appData.bind(this), this.appDataLength())
    );
  }


  unpackTo(_o: ConnectClientMessageT): void {
    _o.connectionString = this.connectionString();
    _o.appData = this.bb!.createScalarList<number>(this.appData.bind(this), this.appDataLength());
  }
}

export class ConnectClientMessageT implements flatbuffers.IGeneratedObject {
  constructor(
    public connectionString: string | Uint8Array | null = null,
    public appData: (number)[] = []
  ) { }


  pack(builder: flatbuffers.Builder): flatbuffers.Offset {
    const connectionString = (this.connectionString !== null ? builder.createString(this.connectionString!) : 0);
    const appData = ConnectClientMessage.createAppDataVector(builder, this.appData);

    return ConnectClientMessage.createConnectClientMessage(builder,
      connectionString,
      appData
    );
  }
