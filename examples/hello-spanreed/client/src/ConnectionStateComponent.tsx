    ...
    return (
      ...
      >
        <p style={{ fontWeight: 700, fontSize: '18pt', margin: 0 }}>
          {connection.type === 'websocket' ? 'Websocket' : 'WebTransport'}{' '}
...
const SetupConnectionComponent: React.FC<SetupConnectionComponentProps> =
  React.memo(({ onOpenConnection, logCb }) => {
    ...
    const onClickConnect = React.useCallback(async () => {
      ...
      if (leftSelected) {
        try {
          ...
          log(
            `Error connecting to WebSocket! ${e instanceof Error ? e.message : typeof e === 'string' ? e : '<unknown>'}`,
            LogLevel.Error
          );
        }
      } else {
        try {
          const wtConnection = await connectWebtransport(
            spanUrl,
            destUrl,
            userName,
            logCb ?? DefaultLogCb
          );
          if (wtConnection.type !== 'webtransport') {
            log(`Not a WebTransport connection!`, LogLevel.Warning);
            return;
          }

          onOpenConnection?.(wtConnection);
        } catch (e) {
          log(
            `Error connecting to WebTransport! ${e instanceof Error ? e.message : typeof e === 'string' ? e : '<unknown>'}`,
            LogLevel.Error
          );
        }
      }
      ...
      setLoading(false);
    }, [setLoading, leftSelected, spanUrl, destUrl, userName]);
      ...
      >
          ...
          >
            ...
            <div
              style={{
                ...
                color: '#888',
                ...(!leftSelected && {
                  borderBottom: '30px solid #4538e5',
                  color: '#fff',
                }),
              }}
            >
        ...
        >
          ...
          <input
            ...
            value={userName}
            onChange={(e) =>
              loading ? setUserName(userName) : setUserName(e.target.value)
            }
          ></input>
          ...
          <input
            ...
            value={spanUrl}
            onChange={(e) =>
              loading ? setSpanUrl(spanUrl) : setSpanUrl(e.target.value)
            }
          ></input>
          ...
          <input
            ...
            value={destUrl}
            onChange={(e) =>
              loading ? setDestUrl(destUrl) : setDestUrl(e.target.value)
            }
          ></input>
        </form>

        {/* Submit! */}
        <Button disabled={loading} onClick={onClickConnect}>
          <span>Connect!</span>
        </Button>
      </div>
    );
